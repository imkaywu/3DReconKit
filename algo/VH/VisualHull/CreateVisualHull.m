...
for i = 1:size(params,3)
    ...
    object_points_cam(object_points_cam <= 0) = 1; % the first row and column has to be black
    ...
    ind = sub2ind(img_size(1:2), object_points_cam(2,:)',object_points_cam(1,:)');
    cur_silhouette = silhouettes(:,:,i);
    img_val = cur_silhouette(uint32(ind)); % the first pixel has to be black
    voxels(1:numel(img_val), 4) = voxels(1:numel(img_val), 4) + double(img_val(:));
