...
if __name__ == "__main__":
    ...
    for cam_ind in range(0, nimages):
        cam = bpy.data.objects['Camera.%03d' % cam_ind]
        P, _, _ = get_3x4_P_matrix_from_blender(cam)
        nP = numpy.matrix(P)
        fid = open("%s/%04d.txt" % (odir, cam_ind), 'wt')
        fid.write('CONTOUR\n')
        for r in range(0, 3):
            fid.write('%.04f %.04f %.04f %.04f\n' % (nP.item(r, 0), nP.item(r, 1), nP.item(r, 2), nP.item(r, 3)))
        fid.close();