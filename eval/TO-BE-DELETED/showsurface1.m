function ptch = showsurface1( voxels )
...
ux = xlim(1) : voxels.Resolution : xlim(2);
uy = ylim(1) : voxels.Resolution : ylim(2);
uz = zlim(1) : voxels.Resolution : zlim(2);
...
for ii=1:N
    [~, ix] = min(abs(ux - voxels.XData(ii)));
    [~, iy] = min(abs(uy - voxels.YData(ii)));
    [~, iz] = min(abs(uz - voxels.ZData(ii)));
