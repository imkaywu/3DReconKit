...
template<typename Ty>
void mineucdiff(const Ty* X1, const Ty* X2, Ty* M, int d, int n1, int n2)
{
	const Ty* v1 = X1;
	Ty s;
	// for (int i = 0; i < 10; ++i)
	// {
	// 	std::cout << '(' << *(X1 + 3 * i) << ',' << *(X1 + 3 * i + 1) << ',' << *(X1 + 3 * i + 2) << ')' << std::endl;
	// }
	// for (int i = 0; i < 10; ++i)
	// {
	// 	std::cout << '(' << *(X2 + 3 * i) << ',' << *(X2 + 3 * i + 1) << ',' << *(X2 + 3 * i + 2) << ')' << std::endl;
	// }
	for(int i = 0; i < n1; ++i)
	{
		s = 1000; // a very large number
		const Ty* v2 = X2;
		for(int j = 0; j < n2; ++j)
		{
			Ty cv = 0;
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    if (mxGetClassID(mxX1) == mxDOUBLE_CLASS)
    {
    	mxArray* mxD;
    	if (opcode != 9)
        	mxD = mxCreateNumericMatrix(n1, n2, mxDOUBLE_CLASS, mxREAL);
        else
        {
        	mxD = mxCreateNumericMatrix(n1, 1, mxDOUBLE_CLASS, mxREAL);
        }
